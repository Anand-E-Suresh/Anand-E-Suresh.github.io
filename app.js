...
// ══ THEME LOADER ══
// Called by every page on load — applies saved colors/fonts from Firebase
async function applyTheme() {
  const d = await getSiteData('theme');
  if (!d) return;
  const root = document.documentElement;
  const vars = ['bg','bg2','bg3','card','border','border2','accent','accent2','accent3','text','muted','white'];
  vars.forEach(function(v) { if (d[v]) root.style.setProperty('--'+v, d[v]); });
  // Fonts
  if (d.fontDisplay || d.fontBody || d.fontMono) {
    // Inject Google Fonts link if custom fonts selected
    const fonts = [d.fontDisplay, d.fontBody, d.fontMono].filter(Boolean);
    const gFonts = fonts.filter(function(f) { return f && f !== 'system-ui' && f !== 'Georgia' && f !== 'monospace'; });
    if (gFonts.length) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = 'https://fonts.googleapis.com/css2?'+gFonts.map(function(f) {
        return 'family='+f.replace(/ /g,'+') + ':wght@300;400;500;600;700';
      }).join('&')+'&display=swap';
      document.head.appendChild(link);
    }
    if (d.fontDisplay) root.style.setProperty('--font-display', "'"+d.fontDisplay+"', Georgia, serif");
    if (d.fontBody)    root.style.setProperty('--font-body',    "'"+d.fontBody+"', system-ui, sans-serif");
    if (d.fontMono)    root.style.setProperty('--font-mono',    "'"+d.fontMono+"', monospace");
  }
}
